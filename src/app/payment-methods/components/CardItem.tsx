...
type Props = {
    card: CardData; onClick: () => void; index: number;
};

const getCardBrandIcon = (cardNumber: string) => {
    if (/^4/.test(cardNumber)) return <FontAwesomeIcon icon={faCcVisa} className="text-2xl"/>;

    if (/^5[1-5]/.test(cardNumber)) return <FontAwesomeIcon icon={faCcMastercard} className="text-2xl"/>;

    if (/^3[47]/.test(cardNumber)) return <FontAwesomeIcon icon={faCcAmex} className="text-2xl"/>;

    if (/^6(?:011|5)/.test(cardNumber)) return <FontAwesomeIcon icon={faCcDiscover} className="text-2xl"/>;
